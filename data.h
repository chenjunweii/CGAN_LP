...
namespace data{
	...
	class db{
		...
		public:
			...
			/*lmdb::env env; //= lmdb::env::create(); // global pointer;
			...
			vector <annotation> target; // vector stores the loaded annotation
			...
			inline int load_detection(vector <int>);

			inline int load_generative(vector <int>);
	...
	inline void MatVector_to_NDArray(vector <cv::Mat> &v, NDArray &n);
	
	inline NDArray MatVector_to_NDArray(vector <cv::Mat> v, Context device);

	inline vector <cv::Mat> NDArray_to_MatVector(NDArray);
